function record_screen_gif
    ...
    if pgrep $target_process > /dev/null
        ...
        notify-send -i ~/.config/fish/icons/camera_gif_icon.png -r $(cd ~/Pictures/Records/ && ls -1 | wc -l) "Recording Stopped 󰙧 (GIF)" -t 2000
    else
        set geometry (slurp)
        if not [ -z $geometry ]
            set record_name $(echo "recrod-$(date +"%Y-%m-%d--%H:%M:%S").gif")
            notify-send -i ~/.config/fish/icons/camera_gif_icon.png -r $(cd ~/Pictures/Records/ && ls -1 | wc -l) "Recording Started  (GIF)" -t 2000
